...
import os
import re
from urllib.parse import unquote_plus  # Import added
...
def lambda_handler(event, context):
    ...
    for record in event['Records']:
        print(f"Which record we are working on {record}")
        try:
            # Extract bucket and object details from the event
            bucket_name = record['s3']['bucket']['name']
            object_key = unquote_plus(record['s3']['object']['key'])  # Use unquote_plus to decode the object key

            # Extract folder name and file name from the object key
            folder_name, file_name = os.path.split(object_key)

            print(f"The file that is getting processed is ####### {file_name}")
